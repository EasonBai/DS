...
/*
func (svr *Server) handleupdate(args *C2SArgs, reply *C2SReply) {
	filename:=args.Filename
	cid:=args.ClientId
	Rid:=args.ReqId
	key:=filename+"//"+strconv.Itoa(cid)
	...
	// 		fmt.Println("Server not back yet")
	// 		break
	// 	}
	// }
}
...
func (svr *Server) BackupTables(args M2BArgs, reply *M2BReply) error{
	...
	case 6:
		fmt.Println("case 6")
		svr.ClientList[args.ClientId]=ClientEnd{Id: args.ClientId, IP:args.ClientIp, LocalPort:args.ClientPort}
		svr.AllClientsId[args.ClientId]=struct{}{}
		if _,contains :=svr.ClientOpTable[args.ClientId]; !contains{
			svr.ClientOpTable[args.ClientId]=make(map[string]struct{})
		}
...
func (svr *Server)StartServer(me int, isMaster bool, port string, address PeerAddress) {
...
func (svr *Server) Enter (args C2SArgs, reply *C2SReply) error{
	...
	m2bArgs := M2BArgs{UpdateType: 6, ClientId: args.ClientId, ClientIp: args.ClientIp, ClientPort: args.ClientPort}
	m2bReply := &M2BReply{}
	fmt.Printf("Client id: %v before sendRecordToBackup\n", args.ClientId)
	ok:=svr.sendRecordToBackup(svr.peer.IP, svr.peer.Port, m2bArgs, m2bReply)
	fmt.Println(ok)
	fmt.Printf("Client id: %v after sendRecordToBackup && before sendOnlineClienttoall\n", args.ClientId)
	//add the client to the onlineclientlist
	go svr.sendClientsToOnlineClients()
...
func (svr *Server) sendClientsToOnlineClients(){
	for k:=range svr.OnlineClientList{
		args := S2CArgs{OpType: "CLIST", AllClients: svr.AllClientsId, OnlineClients: svr.OnlineClientList}
		reply := &S2CReply{}
		client, err := rpc.DialHTTP("tcp", svr.ClientList[k].IP+":"+svr.ClientList[k].LocalPort)
		if err != nil{
			fmt.Println("dialing error")
		}
	...
	select{
		...
		case <-time.After(time.Duration(timeoutDuration) * time.Millisecond):
			...
			if (!svr.masterIsAlive()) && svr.Check(){
				...
				for k := range svr.OnlineClientList{
					svr.sendChangeMasterToClients(svr.me, svr.ClientList[k].IP, svr.ClientList[k].LocalPort)
				}
				svr.mu.Unlock()
			}
	}
...
func (svr *Server) sendChangeMasterToClients(newMasterId int, clientIP string, clientPort string){
	client, err := rpc.DialHTTP("tcp", clientIP+":"+clientPort)
	if err != nil{
		fmt.Println("dialing error")
	}

	args := &S2CArgs{OpType:"CHANGE", NewMasterId: newMasterId}
	reply := &S2CReply{}
	err = client.Call("Client.Operate", args, reply)
...
func (svr *Server)Check() bool{
	svr.ClientNumNotConMstr = 0
	for i :=range svr.OnlineClientList {
		fmt.Println(svr.ClientList[i].LocalPort)
		svr.CheckIfMasterDown(svr.ClientList[i])
	}
	if svr.ClientNumNotConMstr == len(svr.OnlineClientList){
		fmt.Println(svr.ClientNumNotConMstr)
		fmt.Println(len(svr.OnlineClientList))
		return true
	}
	return false
}
