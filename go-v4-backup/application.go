...
func (svr *Server) BackupTables(args M2BArgs, reply *M2BReply) error{
	...
	fmt.Println("now get a command from the master")
	svr.mu.Lock()
	defer svr.mu.Unlock()

	fmt.Println("now switching case")
...
func sayhelloName(w http.ResponseWriter, r *http.Request) {
    ...
    fmt.Fprintf(w, "This is for Backup, Now Start Listening\n") // send data to client side
	fmt.Fprintf(w, pp)
...
func main() {
	...
	svr.timestamp=1
	svr.peer=PeerAddress{IP:"34.210.51.68", Port:"5000"}
	svr.killed=false
...
func (svr *Server) printTables(){
	fmt.Println("-------------------Print backup tables---------------------")
	//FileOp
