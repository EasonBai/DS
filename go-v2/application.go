...
func sayhelloName(w http.ResponseWriter, r *http.Request) {
    ...
    fmt.Fprintf(w, "This is for server, Now Start Listening\n") // send data to client side
	fmt.Fprintf(w, pp)
...
func main() {
	...
	svr.timestamp=1
		paFor0 := PeerAddress{IP:"34.209.252.129", Port:"5000"}
	svr.peer = paFor0
	svr.killed=false
...
func (svr *Server)doStaff(){
	for !svr.killed{
		if svr.isMaster {
			//fmt.Println("is master, doing stuff")
...
func (svr *Server) sendEntrytoClients(){
	...
	for id:=range svr.ClientList{
		...
		if len(svr.ClientOpTable[id])>0 {
			fmt.Println("now send to id %v",id)
			//go func(){
